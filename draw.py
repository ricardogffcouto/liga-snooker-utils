...
amount_of_players_per_group = 4
amount_of_players_in_first_division = 8
amount_of_divisions = 2
amount_of_promoted_players = 4
amount_of_seeds = amount_of_players_per_group

# Mudar esta variaveis:
amount_of_first_division_players_signed_in = 8

# Criar participantes "dummy". Só interessa ter uma lista de participantes no participants
...
with open('players_for_draw', newline='') as csvfile:
    ...
    for part in spamreader:
        participants.append(Participant(part[1], int(part[0]), int(part[2])))
...
for p in participants:
    ...
    if len(participants_in_divisions[0]) == amount_of_players_in_first_division:
        break
...
for g in range(int(len(participants_in_divisions[0]) / amount_of_players_per_group)):
    seeds_for_promotion_playoff[0].append(Place(place=3, division=1, group=string.ascii_uppercase[g]))

for g in range(int(len(participants_in_divisions[1]) / amount_of_players_per_group)):
    seeds_for_promotion_playoff[1].append(Place(place=1, division=2, group=string.ascii_uppercase[g]))
    seeds_for_promotion_playoff[2].append(Place(place=2, division=2, group=string.ascii_uppercase[g]))
...
for bye in range(amount_of_byes):
    seeds_for_promotion_playoff[3].append(Participant("Bye", 99, 2))
...
amount_of_promotion_playoff_rounds = int(math.log(len(all_players_in_promotion_playoff)) / math.log(2) - 2)
...
print("\x1b[1;33mPLAYOFF DE SUBIDA\n")

initial_game_number = 0
for r in range(amount_of_promotion_playoff_rounds):
    print("\x1b[1;37mRonda {}".format(r + 1))
    if r == 0:
...
def alocate_participants_in_seeds(participants, division_order):
    seed_groups = [[] for x in range(amount_of_seeds)]

    players = sorted(participants, key=lambda object1: object1.ranking)

    amount_of_players_per_seed = math.ceil(len(players) / amount_of_seeds)
